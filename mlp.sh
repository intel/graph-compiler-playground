...
do
  ...
  do
      echo "Benchmark $size"
      benchmark-run -b mlp -p "name='${size}'" --dtype "${DTYPE}" -bs 1024 --benchmark_desc "${size}_bs1024" ${DL_BENCH_ARGS} || echo Failed
  done
  ...
  do
      ...
      benchmark-run -b mlp -p "name='${size}'" -bs ${BATCH_SIZE} --dtype "${DTYPE}" --benchmark_desc "${size}_bs${BATCH_SIZE_TXT}" ${DL_BENCH_ARGS} || echo Failed
  done
done
